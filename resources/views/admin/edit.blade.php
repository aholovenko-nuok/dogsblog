    ...
    <div id="content-wrapper">
      ...
      <div class="container-fluid">
        ...
        <div class="card mb-3" style="padding: 50px 0; border: none;">
          <form class="col-md-8" style="margin: 0 auto" method="POST" action="/admin/places/{{$place->id}}">
            {{ method_field('PATCH')}}
            {{ csrf_field() }}
            <div class="form-group">
              <label>Назва пам'ятки</label>
              <input type="text" name="title" class="form-control" value="{{$place->title}}">
            </div>
            <div class="form-group">
              <label>Контент</label>
              <textarea name="content" class="form-control" id="exampleFormControlTextarea1" rows="7">{{$place->content}}</textarea>
            </div>
            <div class="form-group">
              <label>Категорія</label>
              <select name="cat_id" class="form-control form-control">
                  @foreach($categories as $cat)
                  @if($place->cat_id == $cat->id)
